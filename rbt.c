...
print_buffer* create_buffer(int height) {
    print_buffer *pb = (print_buffer *)malloc(sizeof(print_buffer)); //CHANGED
    pb->height = height * 3;  // Increased height multiplier
    pb->width = 1 << (height + 3);  // Increased width
    pb->buffer = (char **)malloc(pb->height * sizeof(char*)); //CHANGED
    
    for (int i = 0; i < pb->height; i++) {
        pb->buffer[i] = (char *)calloc(pb->width + 1, sizeof(char));
